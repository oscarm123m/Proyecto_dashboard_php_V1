<?php 
  ...
  if (!empty($_POST)) {
    ...
    if ($_POST['action']=='addClientePro') {
      $idcliente=$_POST['idCliente_pro'];
      $documento=$_POST['txt_descripcion'];
      $fecha=$_POST['fec_cliente'];
      $abono=$_POST['abo_cliente'];
      $saldo=$_POST['sal_cliente'];
      $saldo_pend=$saldo-$abono;
      ...
      if($idcliente!=''){
      $query_insert=mysqli_query($con,"INSERT INTO cuentapendiente(IdProveedor, SaldoPagar, ValorFacura, Fecha, Descripcion, Estatus,SaldoPendiente,Estado) VALUES('$idcliente','$abono','$saldo','$fecha', '$documento',1,'$saldo_pend','Se debe')");
        if ($query_insert) {
        $codCliente=mysql_insert_id($con);
        $msg=$codCliente;
        } else {
        ...
        $query_insert=mysqli_query($con,"INSERT INTO cuentapendiente( SaldoPagar, ValorFacura, Fecha, Descripcion, Estatus,SaldoPendiente,Estado) VALUES('$abono','$saldo','$fecha', '$documento',1,'$saldo_pend','Se debe')");
        if ($query_insert) {
        $codCliente=mysql_insert_id($con);
        $msg=$codCliente;
        } else {
    ...
    if ($_POST['action']=='searchCliente_pro') {
        $id_cliente=$_POST['cliente'];

        $query=mysqli_query($con,"SELECT IdProveedor FROM proveedor WHERE IdProveedor=$id_cliente");
        $result=mysqli_num_rows($query);
        $data='';
        if ($result>0) {
          $data=mysqli_fetch_assoc($query);
